...
- The page still fetches after mount instead of using a route load, which keeps the guest-session flow straightforward for now.
...
- To find guest-session bootstrapping visit [../../lib/guest-session.ts](file:///C:/Hackathons/Pacific%20Prediction/Frontend/src/lib/guest-session.ts).
