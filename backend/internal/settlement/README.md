...
- Settlement completion should publish `market.updated` and `market.settled` events only after the transaction commits so stream consumers only see committed outcomes.
...
- To find settlement realtime publishing visit [service_impl.go](file:///C:/Hackathons/Pacific%20Prediction/backend/internal/settlement/service_impl.go).
...
- The settlement stream handoff can be found in [../realtime/README.md](file:///C:/Hackathons/Pacific%20Prediction/backend/internal/realtime/README.md).
